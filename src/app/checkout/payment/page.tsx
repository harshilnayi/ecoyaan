...
import { Banknote, CheckCircle, CreditCard, Loader2, ShieldCheck, Smartphone } from 'lucide-react';
...
export default function PaymentPage() {
  ...
  return (
    <>
      <div className="container relative mx-auto flex-grow max-w-6xl px-4 py-6 pb-32 lg:py-10 lg:pb-36">
        {isProcessing && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/85 backdrop-blur-sm animate-fadeInUp">
        ...
        <div className="surface-card animate-fadeInUp p-5 md:p-6">
          <div className="border-b border-green-100 pb-5">
            <p className="text-xs font-semibold tracking-[0.15em] uppercase text-green-700">Step 3</p>
            <h1 className="mt-1 text-2xl font-bold text-green-950 md:text-3xl">Payment and Confirmation</h1>
            <p className="mt-1 text-sm text-gray-600">Review order details and choose your payment method.</p>
          </div>
          ...
          <div className="mt-6 flex flex-col gap-8 lg:flex-row">
            <div className="space-y-5 lg:w-2/3">
              <div className="glass-card animate-fadeInUp rounded-2xl p-5 md:p-6">
                ...
                <div className="rounded-xl bg-green-50/80 p-4 text-sm text-gray-700">
                  ...
                  <p className="text-gray-500">
                    {selectedShippingAddress.email} | +91 {selectedShippingAddress.phoneNumber}
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  {cartItems.map((item) => (
                    <div key={item.product_id} className="flex items-center gap-3 rounded-xl border border-green-50 bg-white/60 p-3">
                      ...
                      <div className="text-sm font-bold text-green-700">Rs {item.product_price * item.quantity}</div>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  {paymentMethods.map((method) => (
                    ...
                    >
                      ...
                      />
                      <p className={`text-sm font-semibold ${selectedMethod === method.id ? 'text-green-700' : 'text-gray-700'}`}>
                        {method.label}
                      </p>
              ...
              <div className="glass-card sticky top-24 rounded-2xl p-5">
                ...
                <div className="space-y-3 text-sm text-gray-600">
                  ...
                  <div className="mt-3 border-t border-green-100 pt-3">
                    ...
                    {couponAmount > 0 && (
                      <p className="mt-1 text-right text-xs font-medium text-green-600">You saved Rs {couponAmount}</p>
                    )}
                  </div>
                </div>

                <p className="mt-4 text-xs text-gray-500">Payments are simulated for this assignment. No real transaction is charged.</p>
              </div>
            </div>
          </div>
        </div>
